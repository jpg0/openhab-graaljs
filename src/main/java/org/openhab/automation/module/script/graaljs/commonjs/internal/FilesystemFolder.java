...
public class FilesystemFolder extends AbstractFolder {
  ...
  private FilesystemFolder(File root, @Nullable Folder parent, String path, String encoding) {
    super(parent, path);
    this.root = root;
  ...
  public Optional<Folder> getFolder(String name) {
    ...
    if (folder.exists()) {
      return Optional.of(new FilesystemFolder(folder, this, getPath() + name + File.separator, encoding));
    } else {
      return Optional.empty();
    }
  }
