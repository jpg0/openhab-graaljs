 ...
 */
@NonNullByDefault
public class JSModule implements RequireFunction, ScriptModule {
    ...
    private @Nullable ModuleLocator moduleLocator;

    private JSModule top;
    private Value module;
    private List<Value> children = new ArrayList<>();

    private List<BiConsumer<String, String>> dependencyListeners = new ArrayList<>();
    ...
    private Logger logger = LoggerFactory.getLogger(JSModule.class);
    ...
    public JSModule(
            ...
            Value module,
            Value exports,
            @Nullable JSModule parent,
            @Nullable JSModule top,
            @Nullable ModuleLocator moduleLocator)
    ...
    public Value require(@Nullable String module) throws PolyglotException {
        ...
        if (module == null) {
            ...
            Optional<? extends ScriptModule> found = Optional.empty();

            //first we defer to a supplied module locator, if we have one
            if(moduleLocator != null) {
                found = moduleLocator.locateModule(module);
                //if found, add to the cache
                found.ifPresent(m -> cache.put(module, m));
            }
            ...
            if(!found.isPresent()) {
                ...
                try {
                    ...
                    if (!found.isPresent() && isPrefixedModuleName(module)) {
            ...
            return found.map(ScriptModule::getExports).orElseThrow(() -> throwModuleNotFoundException(module));
        }

        throw new InternalError("Failed to throw correct exception!");
    }

    private void addDependencyModule(ScriptModule scriptModule) {
        children.add(scriptModule.getModule());
        if(scriptModule instanceof JSModule) {
            JSModule jsModule = ((JSModule) scriptModule);
            jsModule.addDependencyListener(this::notifyDependencyAdded);
            notifyDependencyAdded(this.filename, jsModule.filename);
        }
    }

    private void notifyDependencyAdded(String from, String to) {
        dependencyListeners.forEach(c -> c.accept(from, to));
    }

    private Optional<ScriptModule> searchForModuleInPath(
    ...
    private Optional<ScriptModule> searchForModuleInNodeModules(
            Folder resolvedFolder, String[] folderParts, String filename) throws PolyglotException {
        ...
        while (current.isPresent()) {
            ...
            Optional<ScriptModule> found = nodeModules
    ...
    private Optional<ScriptModule> attemptToLoadFromThisFolder(Folder resolvedFolder, String filename)
            throws PolyglotException {
        ...
        Optional<ScriptModule> found = Optional.ofNullable(cache.get(requestedFullPath));
        if (found.isPresent()) {
    ...
    private Optional<ScriptModule> loadModuleAsFile(Folder parent, String filename) throws PolyglotException {
    ...
    private Optional<ScriptModule> loadModuleAsFolder(Folder parent, String name) throws PolyglotException {
    ...
    private Optional<ScriptModule> loadModuleThroughPackageJson(Folder parent) throws PolyglotException {
    ...
    private Optional<ScriptModule> loadModuleThroughIndexJs(Folder parent) throws PolyglotException {
    ...
    private Optional<ScriptModule> loadModuleThroughIndexJson(Folder parent) throws PolyglotException {
    ...
    private Optional<ScriptModule> compileModuleAndPutInCache(Folder parent, String fullPath, String code)
            throws PolyglotException {
    ...
    private JSModule compileJavaScriptModule(Folder parent, String fullPath, String code)
            throws PolyglotException {
        ...
        JSModule created = new JSModule(context, parent, modulePaths, cache, fullPath, module, exports, this, this.top, this.moduleLocator);
        addDependencyModule(created);
    ...
    private JSModule compileJsonModule(Folder parent, String fullPath, String code)
            throws PolyglotException {
        Value module = createNewObject();
    ...
    private <T> T throwModuleNotFoundException(String module) throws PolyglotException {
        ...
        throw new InternalError("Failed to throw script exception");
    }
    ...
    public void addDependencyListener(BiConsumer<String, String> dependencyListener) {
